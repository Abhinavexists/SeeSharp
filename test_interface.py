...
class ERSVRTester:
    ...
    def create_comparison_visualization(self, input_frame, sr_output, save_path=None):
        ...
        input_upsampled = cv2.resize(
            input_img, (sr_img.shape[1], sr_img.shape[0]), interpolation=cv2.INTER_CUBIC
        )
    ...
    def test_frame_sequence(self, frame_paths, output_dir="results"):
        ...
        comparison_path = os.path.join(
            output_dir, f"{base_name}_sequence_comparison.png"
        )
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="student_models/student_best.pth",
    ...
    parser.add_argument(
        "--device", type=str, choices=["cpu", "cuda"], help="Device to run inference on"
    )
    ...
    if args.image:
        ...
        print(
            "Please provide either --image for single image testing or --frames for sequence testing"
        )
        print("\nExample usage:")
