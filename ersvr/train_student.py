...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for lr_frames, hr_frame in dataloader:
            ...
            hr_frame_up = F.interpolate(
                hr_frame, scale_factor=4, mode="bicubic", align_corners=False
            )
...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, default="./archive", help="Path to dataset"
    )
    parser.add_argument(
        "--teacher_ckpt",
        type=str,
        default="../teacher_models/ersvr_best.pth",
        help="Path to teacher checkpoint",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="./student_checkpoints",
        help="Where to save student models",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.7, help="Distillation loss weight"
    )
    parser.add_argument(
        "--max_sequences",
        type=int,
        default=None,
        help="Max sequences for fast training",
    )
    parser.add_argument(
        "--full_dataset",
        action="store_true",
        help="Use full dataset (overrides max_sequences)",
    )
    ...
    if args.full_dataset:
        ...
        train_dataset = VimeoDataset(
            args.data_path, sample_size=args.max_sequences, verbose=True
        )
        val_dataset = VimeoDataset(
            args.data_path,
            sample_size=int(args.max_sequences * 0.1) if args.max_sequences else 100,
            verbose=False,
        )
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch + 1}/{args.epochs}") as pbar:
            for lr_frames, hr_frame in pbar:
                ...
                loss = distillation_loss(
                    student_out, teacher_out, hr_frame_up, alpha=args.alpha
                )
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                total_loss += loss.item() * lr_frames.size(0)
                pbar.set_postfix(
                    {"loss": total_loss / ((pbar.n + 1) * args.batch_size)}
                )
        # Validation
        val_loss, val_ssim, val_psnr = validate(student, val_loader, device)
        print(
            f"Validation loss: {val_loss:.4f}, SSIM: {val_ssim:.4f}, PSNR: {val_psnr:.2f}"
        )
        # Save best
        if val_ssim > best_ssim:
            ...
            torch.save(
                ...
                os.path.join(args.output_path, "student_best.pth"),
            )
