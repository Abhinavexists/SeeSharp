...
def show_dataset_info(dataset_path):
    ...
    if os.path.exists(vimeo_dir) and os.path.isdir(vimeo_dir):
        print("Found vimeo_settuplet_1 directory")
        ...
        if os.path.exists(sequences_dir) and os.path.isdir(sequences_dir):
            ...
            if seq_folders:
                ...
                if subdirs:
                    ...
                    print(
                        f"Contents of {seq_folders[0]}/{subdirs[0]}: {os.listdir(sample_subdir)}"
                    )

        else:
            print("No sequences directory found, checking vimeo_settuplet_1 directly")
            image_files = [f for f in os.listdir(vimeo_dir) if f.endswith(".png")]
            if image_files:
                print(
                    f"Found {len(image_files)} image files in vimeo_settuplet_1, e.g.: {image_files[:5]}"
                )

            subdirs = [
                d
                for d in os.listdir(vimeo_dir)
                if os.path.isdir(os.path.join(vimeo_dir, d))
            ]
            if subdirs:
...
def visualize_sample(dataset):
    ...
    axes[3].set_title("HR Frame (Target)")
    axes[3].axis("off")
