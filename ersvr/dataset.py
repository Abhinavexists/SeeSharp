        ...
        if split_list is not None and os.path.exists(split_list):
            ...
            with open(split_list) as f:
                ...
                for line in lines:
                    ...
                    if os.path.exists(seq_path) and self._check_sequence_valid(
                        seq_path
                    ):
            ...
            if self._check_sequence_valid(seq_dir):
                ...
                numeric_dirs = [
                    d
                    for d in subdirs
                    if d.isdigit() or (len(d) >= 5 and d[:5].isdigit())
                ]

                if numeric_dirs and verbose:
                    print(
                        f"Found {len(numeric_dirs)} numeric directories (e.g., {numeric_dirs[:5]})"
                    )
                ...
                if numeric_dirs:
                    ...
                    if verbose:
                        print("Scanning subdirectories for sequences...")
