...
class VimeoDataset(Dataset):
    ...
    def _load_from_split_list(self, split_list, seq_dir, max_sequences=None):
        ...
        with open(split_list) as f:
            ...
            if max_sequences:
                ...
                if self.verbose:
                    print(
                        f"Limiting to first {max_sequences} sequences from split list"
                    )
    ...
    def _scan_directory_structure(self, seq_dir, max_sequences=None):
        ...
        if self._check_sequence_valid(seq_dir):
            ...
            numeric_dirs = [
                d for d in subdirs if d.isdigit() or (len(d) >= 5 and d[:5].isdigit())
            ]
                ...
                for dir_name in numeric_dirs:
                    ...
                    if self._check_sequence_valid(dir_path):
                        ...
                        for subdir in os.listdir(dir_path):
                            ...
                            if os.path.isdir(
                                subdir_path
                            ) and self._check_sequence_valid(subdir_path):
                                self.sequences.append(subdir_path)
                                count += 1
            else:
                self._scan_for_sequences(
                    seq_dir, depth=0, max_depth=3, max_sequences=max_sequences
                )
        ...
        try:
            for item in os.listdir(directory):
                ...
                if os.path.isdir(item_path):
                    self._scan_for_sequences(
                        item_path, depth + 1, max_depth, max_sequences
                    )
